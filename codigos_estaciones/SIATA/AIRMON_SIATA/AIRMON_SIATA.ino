...
const char *ssid = "NombreDelWiFi"; //Sustituya NOMBRE_WiFi por el nombre de su red WiFi-
const char *password = "EscribaContraseña";    //Sustituya Contraseña_WiFi por su contraseña de red WiFi
//PIXELS, PIN, NEO_GRB + NEO_KHZ800);

#define DELAYVAL 500 // Time
//credenciales para publicar hacia el mqtt broker
const char *user = "aquality";
const char *passwd = "$Air333";
const char *clientID = "airmonq_s015";

//Nombre del Servidor MQTT
//const char *mqtt_server = "galiot.galileo.edu"; //Server Galileo 
const char *mqtt_server = "159.203.104.87"; // Server OCEAN 
//Modificar al nombre que se asigne en el dashboard.    wifi_med_213 es la contraseña
#define TEAM_NAME "airmon/S015"
...
//  pixels.begin(); // INITIALIZE NeoPixel strip object (REQUIRED)
  ...
  while (WiFi.status() != WL_CONNECTED)
  {
    ...
    if (cont > 15)
    {
      Serial.println("Se va reiniciar");
      ESP.restart();
    }
  }
  Serial.println("");
...
/*
 ...
 // Serial.println(output);
  ...
  if (!particulas.begin_I2C())      // Para sensor I2C
    //pmSerial.begin(9600);
    //if (! particulas.begin_UART(&pmSerial))  // Para sensor Serial
